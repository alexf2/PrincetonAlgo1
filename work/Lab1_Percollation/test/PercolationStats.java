...
public class PercolationStats {
    ...
    {    
        if (N < 1)
            throw new IndexOutOfBoundsException(String.format("N has invalid value '%d', should be >= 1.", N));
        
        if (T < 1)
            throw new IndexOutOfBoundsException(String.format("T has invalid value '%d', should be >= 1.", T));
    ...
    {        
        if (args.length == 2){
            ...
            if (n == null || t == null)
            {
                StdOut.printf("Arguments are not integers: %s %s", args[0], args[1]);
                return;
            }
            ...
            StdOut.printf("%% java PercolationStats %d %d%n", n, t);
            ...
            StdOut.println("Bad number of arguments. Specify N and T.");
    }
